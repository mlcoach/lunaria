...
class UserRepository():
    ...
    def get(self, model, filterValue):
        try:
            ...
            if response:
                return response[0]
            raise ErrorModel(ErrorEnum.RECORD_NOT_FOUND,
                             ErrorEnum.RECORD_NOT_FOUND.value)
    ...
    def create(self, user, model, filterValue):
        try:
            ...
            if response:
                raise ErrorModel(ErrorEnum.USER_ALREADY_EXISTS,
                                 ErrorEnum.USER_ALREADY_EXISTS.value)
