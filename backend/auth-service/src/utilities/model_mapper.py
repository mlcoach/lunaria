...
def map_schema(model, to_model):
    same_kwargs = list(set(model.__dict__.keys()) &
                       set(to_model.__dict__.keys()))
    ...
    if to_model == UserLoginModel:
        try:
            ...
            arg_dict['passwordHash'] = hashlib.sha256((model.__getattribute__(
                'password') + arg_dict['passwordSalt']).encode('utf-8')).hexdigest()
        except Exception as e:
            raise ErrorModel(ErrorEnum.PASSWORD_NOT_FOUND,
                             ErrorEnum.PASSWORD_NOT_FOUND.value)
